...
Ключи S3 живут в окружении **контейнера**, а у SSH-сессии окружение своё —
раньше ручной рестарт молча выключал заливку в S3 (`Unable to locate
credentials`, результаты оставались только на поде). Теперь `_inherit_container_env()`
добирает недостающие `AWS_*`/`S3_*` прямо из `/proc/1/environ`, поэтому способ
запуска роли не играет. В логе на старте видно строку «окружение контейнера
добрано у PID 1».
