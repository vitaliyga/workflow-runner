...
| `VIDEO_WAIT_TIMEOUT_S` | жёсткий лимит ожидания video-промта, сек. По умолчанию `0` — ждать до конца (см. «Ожидание генерации») |
| `IMAGE_WAIT_TIMEOUT_S` | то же для картиночных прогонов, по умолчанию `0` |
...
## Ожидание генерации: без таймаута, но не вслепую

Раньше видео-джоба ждала результат максимум 30 минут (`VIDEO_WAIT_TIMEOUT_S=1800`),
а по истечении — помечалась `failed` и **отправлялась в ComfyUI заново**. На
свопящем поде это давало worst case: одно и то же видео уезжало в очередь 2–3
раза, копии делили VRAM, шаг сэмплера растягивался с 8 до 380 с, и генерация уже
физически не могла успеть за отведённые полчаса. Тратилось часы GPU, результата —
ноль.

Теперь по умолчанию лимита нет (`0`): раннер ждёт, пока ComfyUI не отдаст
результат. Вместо будильника два признака **настоящей** поломки — они срабатывают
за минуты, а не за полчаса, и очередь не встаёт:

* **ComfyUI недостижим** — 30 неудачных опросов `/history` подряд (сеть, 5xx,
  403 от proxy, битый JSON). Джоба помечается `stalled`, в логе видно, что ждём;
* **задача пропала** — раз в ~30 с сверяемся с `/queue`. Если промта нет ни в
  running, ни в pending, ни в `/history` (прервали снаружи, ComfyUI
  перезапустился, кто-то очистил очередь) — результата не будет никогда, и
  ожидание завершается ошибкой сразу.

Жёсткий лимит по-прежнему доступен: `VIDEO_WAIT_TIMEOUT_S=3600` вернёт прежнее
поведение с потолком в час.

## Живой прогресс и ETA пакета

`running` без цифр неотличим от «повисло», поэтому раннер подписывается на
websocket ComfyUI (`/ws` с тем же `client_id`, с которым отправлял промт) и
разбирает оба формата прогресса — `progress` (`value`/`max` текущей ноды) и
`progress_state` (состояния всех нод). В таблице видео-прогона на каждую джобу
показываются:

* процент и полоса заполнения; пока ComfyUI грузит модели в VRAM и тиков ещё нет —
  бегущая полоса «подготовка…», а не лживый `0 %`;
* `шаг 2/4` и стадия по заголовку ноды («KSampler», «Декодировать VAE»);
* «осталось ~12м» для этой джобы — из **замеренного** темпа шагов сэмплера
  (EMA-сглаживание, чтобы первый шаг вместе с загрузкой моделей не растянул
  прогноз в разы).

Над таблицей — сводка по пакету: `прошло`, `осталось`, `финиш ≈ HH:MM`,
`на видео` (средняя длительность готовых джоб). Всё считается по факту этого
прогона: средняя из уже готовых джоб + остаток текущей; пока готовых нет, оценка
экстраполируется из процента текущей. Данных не хватает — показывается `—`, а не
выдуманное число. Блок `eta` отдаётся и в `GET /api/runs/{id}/status`.
